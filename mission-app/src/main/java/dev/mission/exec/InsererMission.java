...
public class InsererMission implements CommandLineRunner {
    private final Logger LOGGER = LoggerFactory.getLogger(InsererMission.class);
...
//        launchCreateMission();
    ...
    private void launchCreateMission() {
        ...
        int num = mission.isPresent() ? mission.get().getId() : 0;
        // crée d'autres mission
        for (int i = 0; i < 4; i++) {
            Mission newMission = this.createMission(num++);
